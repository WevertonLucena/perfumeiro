...
Brewer.UploadFoto = (function() {
	...
	function UploadFoto() {
		...
		this.htmlFotoPerfumeTemplate = $('#foto-perfume').html();
		this.template = Handlebars.compile(this.htmlFotoPerfumeTemplate);
		
		this.containerFotoPerfume = $('.js-container-foto-perfume');
	...
	UploadFoto.prototype.iniciar = function () {
		var settings = {
			...
			filelimit: 1,
			allow: '*.(jpg|jpeg|png)',
			action: this.containerFotoPerfume.data('url-fotos'),
	...
	function renderizarFoto(resposta) {
		...
		var htmlFotoPerfume = this.template({url: resposta.url});
		this.containerFotoPerfume.append(htmlFotoPerfume);
		
		$('.js-remove-foto').on('click', onRemoverFoto.bind(this));
	}
	
	function onRemoverFoto() {
		$('.js-foto-perfume').remove();
